# ==========================================================
# FINAL app.py
# VSM SEARCH ENGINE
# Improved Dataset + Score Fix + Better UI
# ==========================================================

import streamlit as st
import pandas as pd
from sklearn.feature_extraction.text import TfidfVectorizer
from sklearn.metrics.pairwise import cosine_similarity

# ----------------------------------------------------------
# PAGE SETTINGS
# ----------------------------------------------------------
st.set_page_config(
    page_title="VSM Search Engine",
    page_icon="🔍",
    layout="wide"
)

# ----------------------------------------------------------
# CUSTOM CSS
# ----------------------------------------------------------
st.markdown("""
<style>
.stApp{
    background-color:#eef4ff;
}

.main-title{
    font-size:52px;
    font-weight:800;
    color:#1f2937;
    margin-bottom:5px;
}

.sub-title{
    font-size:20px;
    color:#475569;
    margin-bottom:25px;
}

.top-box{
    background:#dbeafe;
    padding:22px;
    border-radius:14px;
    border:2px solid #60a5fa;
    margin-bottom:18px;
}

.result-box{
    background:#ffffff;
    padding:18px;
    border-radius:12px;
    border:1px solid #cbd5e1;
    margin-bottom:12px;
}

.rank-title{
    font-size:22px;
    font-weight:700;
    color:#1e3a8a;
}

.small-text{
    color:#475569;
}
</style>
""", unsafe_allow_html=True)

# ----------------------------------------------------------
# DATASET (DOCUMENTS)
# ----------------------------------------------------------
data = [

["Artificial Intelligence","Artificial Intelligence enables machines to mimic human intelligence."],
["Machine Learning","Machine Learning learns patterns from data to make predictions."],
["Deep Learning","Deep Learning uses neural networks with multiple layers."],
["Supervised Learning","Supervised Learning uses labeled training data."],
["Unsupervised Learning","Unsupervised Learning finds hidden patterns in unlabeled data."],
["Reinforcement Learning","Reinforcement Learning learns using rewards and penalties."],

["Regression","Regression predicts continuous values such as salary house price marks."],
["Linear Regression","Linear Regression uses a straight line relationship between variables."],
["Logistic Regression","Logistic Regression is used for binary classification problems."],
["Classification","Classification predicts labels or categories."],

["Decision Tree","Decision Tree uses branches and rules for prediction."],
["Random Forest","Random Forest combines multiple trees for better accuracy."],
["Gradient Boosting","Gradient Boosting builds strong models using weak learners."],
["XGBoost","XGBoost is an optimized boosting algorithm."],
["AdaBoost","AdaBoost combines weak learners sequentially."],

["Support Vector Machine","SVM is used for classification and regression tasks."],
["K Nearest Neighbors","KNN predicts using nearest similar data points."],
["Naive Bayes","Naive Bayes is a probabilistic classifier."],

["Clustering","Clustering groups similar data points using unsupervised learning."],
["K Means","K Means divides data into k clusters."],

["Neural Network","Neural Network uses neurons and layers inspired by brain."],
["Convolutional Neural Network","CNN is used for image recognition and computer vision."],
["Recurrent Neural Network","RNN is used for text speech and sequence data."],
["LSTM","LSTM handles long term dependencies in sequential data."],
["Transformer","Transformer uses attention mechanism in language models."],

["Natural Language Processing","NLP helps computers understand and generate human language."],
["Tokenization","Tokenization splits text into tokens or words."],
["Stopword Removal","Stopword Removal removes common words from text."],
["Stemming","Stemming reduces words to root form."],
["Lemmatization","Lemmatization converts words into meaningful base form."],

["TF IDF","TF IDF measures importance of words in documents."],
["Bag of Words","Bag of Words converts text into frequency vectors."],

["Computer Vision","Computer Vision helps machines understand images and videos."],
["Generative AI","Generative AI creates text images audio or code."],
["Chatbot","Chatbot interacts with users using natural language."],
["Prompt Engineering","Prompt Engineering designs effective prompts for AI models."],

["Confusion Matrix","Confusion Matrix evaluates classification performance."],
["Precision","Precision measures correct positive predictions."],
["Recall","Recall measures captured positive cases."],
["F1 Score","F1 Score balances precision and recall."],

["Epoch","Epoch means one full pass of training data."],
["Batch Size","Batch Size is number of samples processed at once."],
["Activation Function","Activation functions include ReLU sigmoid tanh."],

["Python","Python is popular for machine learning and NLP projects."],
["Pandas","Pandas is used for dataframe and data analysis."],
["NumPy","NumPy is used for arrays and numerical computing."],
["Scikit Learn","Scikit Learn provides machine learning algorithms."]
]

df = pd.DataFrame(data, columns=["title","content"])

# Combine title + content
df["text"] = df["title"] + " " + df["content"]

# ----------------------------------------------------------
# VECTOR SPACE MODEL
# ----------------------------------------------------------
vectorizer = TfidfVectorizer(
    stop_words="english",
    lowercase=True,
    ngram_range=(1,2)
)

document_vectors = vectorizer.fit_transform(df["text"])

# ----------------------------------------------------------
# SEARCH FUNCTION
# ----------------------------------------------------------
def search_engine(query):

    query = query.strip().lower()

    if query == "":
        return None, []

    # Convert query into vector
    query_vector = vectorizer.transform([query])

    # Cosine similarity
    scores = cosine_similarity(query_vector, document_vectors).flatten()

    # Boost common abbreviations
    boost_map = {
        "nlp":"Natural Language Processing",
        "cnn":"Convolutional Neural Network",
        "rnn":"Recurrent Neural Network",
        "svm":"Support Vector Machine",
        "knn":"K Nearest Neighbors",
        "lstm":"LSTM",
        "tfidf":"TF IDF",
        "tf idf":"TF IDF",
        "ai":"Artificial Intelligence",
        "ml":"Machine Learning",
        "dl":"Deep Learning"
    }

    for key, value in boost_map.items():
        if key in query:
            idx = df[df["title"] == value].index[0]
            scores[idx] += 0.60

    # Rank results
    ranked = scores.argsort()[::-1]

    # Threshold
    top_results = [i for i in ranked if scores[i] > 0.22][:5]

    if len(top_results) == 0:
        return "No relevant ML/NLP document matched your query.", []

    # Top Result
    best = top_results[0]

    top_doc = {
        "title": df.iloc[best]["title"],
        "content": df.iloc[best]["content"],
        "score": round(min(scores[best]*100, 99.99),2)
    }

    # Other Results
    others = []

    for i in top_results:
        others.append({
            "title": df.iloc[i]["title"],
            "content": df.iloc[i]["content"],
            "score": round(min(scores[i]*100, 99.99),2)
        })

    return top_doc, others

# ----------------------------------------------------------
# UI
# ----------------------------------------------------------
st.markdown('<div class="main-title">🔍 VSM Based Search Engine</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-title">Search Machine Learning, Deep Learning, NLP and AI topics using Vector Space Model</div>', unsafe_allow_html=True)

query = st.text_input(
    "Enter your query:",
    placeholder="Example: nlp in python / svm / regression / tf idf / deep learning"
)

if st.button("Search", use_container_width=True):

    top_result, results = search_engine(query)

    # No Result
    if isinstance(top_result, str):
        st.error(top_result)

    else:

        # Top Result Box
        st.markdown(f"""
        <div class="top-box">
        <div class="rank-title">🏆 Top Result: {top_result['title']}</div>
        <br>
        <b>Summary:</b> {top_result['content']}<br><br>
        <b>Similarity Score:</b> {top_result['score']}%
        </div>
        """, unsafe_allow_html=True)

        # Relevant Docs
        st.subheader("📄 Relevant Documents")

        rank = 1
        for item in results:
            st.markdown(f"""
            <div class="result-box">
            <b>{rank}. {item['title']}</b> ({item['score']}%)<br>
            <span class="small-text">{item['content']}</span>
            </div>
            """, unsafe_allow_html=True)
            rank += 1
